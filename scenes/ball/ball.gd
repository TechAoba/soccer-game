...
func _ready() -> void:
	...
	GameEvents.kickoff_started.connect(on_kickoff_startd.bind())

func _process(_delta: float) -> void:
	ball_sprite.position = Vector2.UP * height
	scoring_raycast.rotation = velocity.angle()

func switch_state(state: Ball.State, ball_state_data: BallStateData = BallStateData.new()) -> void:
	if current_state != null:
		current_state.queue_free()
	current_state = state_factory.get_fresh_state(state)
	current_state.set_up(self, player_detection_area, carrier, animation_player, ball_sprite, ball_state_data, shot_particles)
