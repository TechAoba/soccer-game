...
func set_up(context_ball: Ball, context_player_detection_area: Area2D
	...
	, context_sprite: Sprite2D, context_state_data: BallStateData) -> void:
...
func transition_state(new_state: Ball.State, ball_state_data: BallStateData = BallStateData.new()) -> void:
	state_transition_requested.emit(new_state, ball_state_data)
