...
var mov_dirs : Dictionary[KeyUtils.Action, Vector2i] = {
...
func _process(_delta: float) -> void:
	for i in range(selectors.size()):
		...
		if not selector.is_selected:
			for action : KeyUtils.Action in mov_dirs.keys():
				if KeyUtils.is_action_just_pressed(selector.control_sceme, action):
					try_navigate(i, mov_dirs[action])
...
func try_navigate(select_idx: int, direction: Vector2i) -> void:
	var rect : Rect2i = Rect2i(0, 0, NB_COLS, NB_ROWS)
	if rect.has_point(selection[select_idx] + direction):
		selection[select_idx] += direction
		var flag_index := selection[select_idx].x + selection[select_idx].y * NB_COLS
		GameManager.player_setup[select_idx] = DataLoader.get_countries()[1 + flag_index]
		selectors[select_idx].position = flags_container.get_child(flag_index).position
		SoundPlayer.play(SoundPlayer.Sound.UI_NAV)
