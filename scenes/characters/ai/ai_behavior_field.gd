...
const PASSING_PROBABILITY := 0.05
const SHOT_DISTANCE := 150
const SHOT_PROBABILITY := 0.3
const SPREAD_ASSIST_FDACTOR := 0.8
	...
	if player.has_ball():
		...
		if total_steering_force.length_squared() < 1:
			if is_ball_possessed_by_opponent():
				total_steering_force += get_spawn_stering_force()
...
func perform_ai_decision() -> void:
	...
	if ball.carrier == player:
		...
		if player.position.distance_to(target) < SHOT_DISTANCE and randf() < SHOT_PROBABILITY:
			...
			var data := PlayerStateData.build().set_shoot_power(player.power).set_shoot_direction(shoot_direction)
			player.switch_state(Player.State.SHOOTING, data)
		elif randf() < PASSING_PROBABILITY and has_opponents_nearby() and has_teammate_in_view():
...
func get_assist_formation_steering_force() -> Vector2:
	...
	var assist_destination := ball.carrier.position - spawn_difference * SPREAD_ASSIST_FDACTOR
...
func get_spawn_stering_force() -> Vector2:
